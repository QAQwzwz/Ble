...
public class MyShanXinView extends View {
    private int[] mColors = {Color.BLUE, Color.DKGRAY, Color.CYAN, Color.RED, Color.GREEN};
    private Paint paint;    //画笔
    private ArrayList<MyShanXinView.ShanXinViewData> shanXinViewDatas;    //数据集
    private int w;          //View宽高
    private int h;
    private RectF rectF;    //矩形

    public MyShanXinView(Context context) {
        super(context);
        initPaint();    //设置画笔
    ...
    //设置数据
    public void setData(ArrayList<MyShanXinView.ShanXinViewData> shanXinViewDatas) {
        this.shanXinViewDatas = shanXinViewDatas;
        initData();     //设置数据的百分度和角度
        invalidate();   //刷新UI
    }

    public MyShanXinView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initPaint();
    }

    public MyShanXinView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; shanXinViewDatas!=null&&i < shanXinViewDatas.size(); i++) {
            ...
            try {
                String name = shanXinViewData.name;
                canvas.drawText(name, x, y, paint);    //绘制文字
